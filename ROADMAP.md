...
| **0.46.6** | Pluggable OCR pipeline + searchable overlay ✅ *(already shipped in 0.29.x — `src/render/ocr.ts`: `OcrEngine`/`applyOcr`/`OcrWord`, invisible `Tr 3` overlay, no bundled weights)* | Define `OcrEngine` interface (image tiles in → words+quads+confidence out, no bundled weights); render rasterized regions, run engine, inject invisible §9.3.3 `Tr 3` text layer aligned to glyph quads producing a searchable overlay. | ~~M~~ done |
